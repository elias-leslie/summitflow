...
- [x] ~~**Event-driven symbol refresh**: index only updates on the bi-hourly sweep; brand-new code invisible up to ~2h~~ — built 2026-06-11 session 6: post-publish targeted reindex of the commit's changed files (see work log). Live-verified: the feature's own publishing commit had its new symbols indexed within seconds, searchable as `mode=symbol-first` without checkout escalation.
