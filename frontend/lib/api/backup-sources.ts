...
export type RestoreOptions = { dry_run?: boolean; db_only?: boolean; files_only?: boolean }
export type BackupListOptions = { limit?: number; offset?: number; status?: string }

function listQuery(options?: BackupListOptions) {
  return buildQueryString({ limit: options?.limit ?? 50, offset: options?.offset, status: options?.status })
}
...
export function updateBackupSource(sourceId: string, data: {
  ...
  return putJson<BackupSource>(`/api/backup-sources/${sourceId}`, data, 'Failed to update backup source')
}
...
export function restoreSourceBackup(sourceId: string, backupId: string, options?: RestoreOptions): Promise<BackupCreateResponse> {
  return postJson<BackupCreateResponse>(
    `/api/backup-sources/${sourceId}/backups/${backupId}/restore`,
    { dry_run: options?.dry_run ?? false, db_only: options?.db_only ?? false, files_only: options?.files_only ?? false },
