...
export function EvidenceBrowser({
  ...
  const handleIframeError = useCallback(() => {
    setIsLoading(false)
    setLoadError(
      'Failed to load page. The page may block embedding (X-Frame-Options).',
    )
  }, [])
    ...
    (result: CaptureResult) => {
      ...
      if (
        ...
        const capture: LastCapture = {
          ...
          capturedAt:
            result.evidence?.metadata?.capturedAt || new Date().toISOString(),
        ...
        setShowPreview(true)
        toast.success(
          `Captured evidence v${result.version} for ${result.capability_id}`,
        )
      }
    },
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm mb-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="mono text-phosphor-400">
                                {lastCapture.capabilityId}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-slate-500">Criterion:</span>
                              <span className="mono text-slate-300">
                                {lastCapture.criterionId}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={
                                  lastCapture.errorCount > 0
                                    ? 'text-rose-400'
                                    : 'text-slate-300'
                                }
                              >
                                {lastCapture.errorCount} errors,{' '}
                                {lastCapture.warningCount} warnings
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                window.open(lastCapture.screenshotUrl, '_blank')
                              }
                              className="gap-1.5"
                            >
