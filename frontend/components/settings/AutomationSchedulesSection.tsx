...
export function AutomationSchedulesSection({
  ...
  const projectSchedules = schedules.filter(
    (schedule) => schedule.scope === 'project',
  )
  const systemSchedules = schedules.filter(
    (schedule) => schedule.scope !== 'project',
  )

  const renderSchedule = (schedule: AutonomousSchedule) => {
    const isPending = updatingScheduleId === schedule.schedule_id
    return (
      <div
        key={schedule.schedule_id}
        className="rounded-lg border border-slate-700/70 bg-slate-900/35 px-4 py-3"
      >
    ...
    <div className="p-6 bg-slate-800/50 rounded-lg border border-slate-700 space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-2">
          Project-level scheduled entrypoints. System jobs are under advanced.
        </p>
      </div>

      <div className="space-y-3">{projectSchedules.map(renderSchedule)}</div>

      {systemSchedules.length > 0 && (
        <details className="group rounded-lg border border-slate-700/60 bg-slate-900/20">
          ...
          <div className="space-y-3 border-t border-slate-700/60 p-4">
            {systemSchedules.map(renderSchedule)}
          </div>
        </details>
      )}
    </div>
