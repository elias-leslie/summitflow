  ...
  return (
    <div
      className={clsx(
        ...
        expanded
          ? 'border-outrun-500/50 shadow-outrun-sm'
      ...
      >
        ...
        <div className="flex items-center gap-4 text-slate-500 text-[12px]">
          {observation.concepts.slice(0, 2).map((concept) => (
            <span
              key={concept}
              className="px-2 py-0.5 rounded-full bg-slate-700/50 text-slate-400"
            >
              {concept}
            </span>
          ))}
        ...
        {expanded && (
          ...
          >
            <div className="px-5 py-4 space-y-4">
              ...
              {observation.facts &&
                Object.keys(observation.facts).length > 0 && (
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-2">
                      Facts Extracted
                    </div>
                    <ul className="space-y-1">
                      {Object.entries(observation.facts).map(([key, value]) => (
                        <li
                          key={key}
                          className="text-sm text-slate-300 pl-4 relative before:absolute before:left-0 before:content-['•'] before:text-outrun-500"
                        >
                          {String(value)}
                        </li>
                      ))}
