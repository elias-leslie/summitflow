...
import { Zap, AlertTriangle, Infinity, Loader2, TrendingUp } from "lucide-react";
import {
...
export function GlobalExtractionPanel() {
  const [settings, setSettings] = useState<GlobalExtractionSettings | null>(null);
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-slate-400 mb-5">
        Limit AI extraction queries globally to control costs. Applies to all projects.
      </p>
      ...
      <div className="flex items-center gap-4 mb-5 text-xs">
        <div className="flex items-center gap-1.5 text-slate-400">
          ...
          <span>Current: <span className="font-mono text-slate-300">{settings.current_rpm}</span>/min</span>
        </div>
        <div className="flex items-center gap-1.5 text-slate-400">
          <span>Today: <span className="font-mono text-slate-300">{settings.requests_today.toLocaleString()}</span> requests</span>
        </div>
      </div>
      ...
      {isOff && (
        <div className="mt-5 p-3 rounded-md bg-red-950/50 border border-red-900/50 flex items-start gap-2">
          ...
          <p className="text-xs text-red-300/90">
            AI extraction is disabled globally. Observations will not be processed until re-enabled.
          </p>
        </div>
      )}
    </div>
