...
import { AlertTriangle, Clock, Loader2, Shield, Trash2 } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-slate-400 mb-5">
        Controls how aggressively unused patterns are cleaned up. Higher = more
        aggressive pruning.
      </p>
      ...
      <div className="flex items-center gap-4 mb-5 text-xs">
        ...
        <div className="flex items-center gap-1.5 text-slate-400">
          <span>
            Min Relevance:{' '}
            <span className="font-mono text-slate-300">
              {(settings.min_relevance * 100).toFixed(0)}%
            </span>
          </span>
        </div>
      </div>
      ...
      {isAggressive && (
        <div className="mt-5 p-3 rounded-md bg-rose-950/50 border border-rose-900/50 flex items-start gap-2">
          ...
          <p className="text-xs text-rose-300/90">
            Aggressive mode: Patterns unused for 7+ days with low relevance will
            be auto-cleaned.
          </p>
        </div>
      )}
      ...
      {isManual && (
        <div className="mt-5 p-3 rounded-md bg-slate-800/50 border border-slate-600/50 flex items-start gap-2">
          ...
          <p className="text-xs text-slate-400">
            Manual mode: Patterns will only be cleaned via explicit
            /memory_optimize runs.
          </p>
        </div>
      )}
    </div>
