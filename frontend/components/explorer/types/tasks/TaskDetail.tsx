...
export function TaskDetail({ entry }: TaskDetailProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Task path */}
      <div>
        <span className="text-xs text-slate-500 uppercase tracking-wide">Task Path</span>
        <p className="font-mono text-sm text-slate-300 mt-1">{meta.task_path || entry.path}</p>
      </div>
      ...
      {meta.schedule_type && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <span className="text-xs text-slate-500 uppercase tracking-wide">Schedule Type</span>
            <p className="text-sm text-slate-200 mt-1 capitalize">{meta.schedule_type}</p>
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Success (7d)</span>
          <p className="font-mono text-sm text-emerald-400 mt-1">
            {formatNumber(meta.success_count_7d)}
          </p>
        </div>
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Failures (7d)</span>
          <p
            className={cn(
              "font-mono text-sm mt-1",
              (meta.failure_count_7d ?? 0) > 0 ? "text-red-400" : "text-slate-400"
            )}
          >
        ...
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Success Rate</span>
          <p
            className={cn(
              "font-mono text-sm mt-1",
        ...
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Avg Duration</span>
      ...
      {meta.last_run_at && (
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Last Run</span>
          <p className="text-sm text-slate-300 mt-1">{formatDate(meta.last_run_at)}</p>
        </div>
      )}
      ...
      {(dependsOn.length > 0 || calledBy.length > 0) && (
        <div className="pt-2 border-t border-slate-700/50">
          <span className="text-xs text-slate-500 uppercase tracking-wide">Dependencies</span>
          <div className="mt-2 space-y-2">
            {dependsOn.length > 0 && (
              <div>
