...
export function TableDetail({ entry }: TableDetailProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Table name */}
      <div>
        <span className="text-xs text-slate-500 uppercase tracking-wide">Table</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Rows</span>
        ...
        <div>
          <span className="text-xs text-slate-500 uppercase tracking-wide">Completeness</span>
          <p
            className={cn(
              "font-mono text-sm mt-1",
              meta.completeness_pct !== undefined && meta.completeness_pct >= 90
                ? "text-emerald-400"
                : meta.completeness_pct !== undefined && meta.completeness_pct < 50
                ? "text-amber-400"
                : "text-slate-200"
            )}
          >
            {meta.completeness_pct !== undefined ? `${Math.round(meta.completeness_pct)}%` : "-"}
          </p>
        </div>
        <div>
