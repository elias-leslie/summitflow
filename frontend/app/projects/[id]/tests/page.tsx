...
import { fetchTddTests, runTddTest, runTddTests, type TddTest } from '@/lib/api'
    ...
    if (statusFilter !== 'all') {
      result = result.filter((t) => {
        ...
        if (statusFilter === 'failed')
          return t.last_result === 'failed' || t.last_result === 'error'
  ...
  return (
    <div className="h-full overflow-auto p-4 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-4">
          <div className="text-2xl font-bold text-phosphor-400 mono">
            {stats.passCount}
          </div>
          <div className="text-sm text-slate-500">Passed</div>
        </div>
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-4">
          <div className="text-2xl font-bold text-rose-400 mono">
            {stats.failCount}
          </div>
          <div className="text-sm text-slate-500">Failed</div>
        </div>
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-4">
          <div className="text-2xl font-bold text-slate-400 mono">
            {stats.pendingCount}
          </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Button
          ...
          disabled={filteredTests.length === 0 || isRunningAll}
        >
          {isRunningAll ? (
