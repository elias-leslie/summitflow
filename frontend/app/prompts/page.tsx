...
export default function CentralizedPromptsPage() {
  ...
  const {
    ...
    queryFn: () => fetchPrompts(selectedProjectId!),
    enabled: !!selectedProjectId,
  })

  // Export mutation
  const exportMutation = useMutation({
    mutationFn: () => exportPrompts(selectedProjectId!),
    onSuccess: (data) => {
      ...
      a.href = url
      a.download = `prompts-${selectedProjectId}-${new Date().toISOString().split('T')[0]}.json`
  ...
  const importMutation = useMutation({
    mutationFn: (promptsToImport: PromptUpdate[]) =>
      importPrompts(selectedProjectId!, promptsToImport),
    onSuccess: (result) => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <section className="animate-in">
        <h1 className="display font-semibold text-2xl text-white flex items-center gap-3 mb-4">
          <FileCode className="w-7 h-7 text-amber-400" />
          Prompts
        </h1>
        <p className="text-slate-400 mb-4">
          Manage prompts for all projects from a single place.
        </p>

        {/* Project Selector */}
        <div className="relative inline-block">
          <button
            data-testid="prompts-project-selector"
            onClick={() => setDropdownOpen(!dropdownOpen)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-700 bg-slate-800/50 hover:bg-slate-800 transition-colors"
          >
              ...
              {projects.map((p) => (
                ...
                >
                  ...
                  <span
                    className={clsx(
                      'text-sm',
                      p.id === selectedProjectId ? 'text-white' : 'text-slate-300',
                    )}
                  >
                    {p.name}
                  </span>
      ...
      {selectedProjectId && !isLoading && (
        <>
          {/* Category Tabs */}
          <div className="flex items-center gap-2 flex-wrap animate-in stagger-1">
            {tabs.map((tab) => {
              const Icon = tab.icon
          ...
          {currentPrompts.length === 0 && (
            <div className="card p-8 text-center">
              ...
              <p className="text-slate-400">No prompts in this category.</p>
            </div>
          )}
        </>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-lg border border-slate-700 max-w-2xl w-full max-h-[80vh] flex flex-col">
            <div className="p-4 border-b border-slate-700 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-white">Import Prompts</h2>
              <button
