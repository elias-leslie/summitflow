...
def execute_subtask_loop(
    ...
    Args:
        ...
        incomplete_subtasks: List of incomplete subtasks
        total_subtasks: Total number of subtasks
        completed_count: Number of already completed subtasks
        task_type: Task type for agent routing
        agent_override: Agent override for routing

    Returns:
        Tuple of (results list, final completed count)
    """
    ...
    for iteration, subtask in enumerate(incomplete_subtasks, 1):
        ...
        if result.get("status") == "failed" and not _handle_subtask_failure(
            task_id, project_id, result, issue_counts, results
        ):
            break
...
def _handle_subtask_failure(
    ...
    """Handle subtask failure with circuit breaker logic.

    Args:
        task_id: The task ID
        project_id: The project ID
        result: Subtask execution result
        issue_counts: Issue frequency tracker
