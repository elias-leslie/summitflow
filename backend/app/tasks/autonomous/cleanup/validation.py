...
def _resolve_check_command(project_id: str) -> list[str] | None:
    """Return configured st check command for post-merge validation."""
...
def _run_check_quick(
    ...
    """Run configured st check validation and classify outcome."""
    from app.storage import log_task_event
...
def run_post_merge_validation(
    ...
    """Run quality checks on merged main branch."""
    from app.storage import log_task_event
...
def _get_head_commit(project_root: str) -> str | None:
    """Return short HEAD commit hash, or None on failure."""
    result = subprocess.run(
...
def _get_merge_affected_files(project_root: str, merge_commit: str) -> list[str]:
    """Return files changed in merge commit."""
    result = subprocess.run(
...
def _apply_rollback_side_effects(
    ...
    *,
    ...
    """Create fix task, update status, save learning after revert."""
    from app.storage import log_task_event
...
def auto_rollback(
    ...
    """Auto-revert merge commit when post-merge validation fails."""
    from app.storage import log_task_event
...
def _create_regression_subtasks(
    ...
    """Create scoped subtasks on regression fix task."""
...
def _build_regression_description(
    ...
    """Build description body for regression fix task."""
    parts = [
...
def _finalize_regression_task(new_task_id: str, task_branch: str, failure_detail: str | None) -> None:
    """Set done_when criteria and subtasks on new regression fix task."""
...
def create_regression_fix_task(
    ...
    """Create regression fix task after rollback."""
...
def save_rollback_learning(
    ...
    try:
        ...
        client.save_learning(
            f"Merge of task {task_id} (branch {task_branch}) was auto-reverted due to post-merge validation failure. Ensure all quality checks pass before merging.",
