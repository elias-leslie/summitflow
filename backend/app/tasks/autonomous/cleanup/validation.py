...
def _get_merge_affected_files(project_root: str, merge_commit: str) -> list[str]:
    """Return files changed in the merge commit (up to 10)."""
    result = subprocess.run(
...
def auto_rollback(
    ...
    Args:
        ...
        failure_detail: Truncated validation failure output for context
...
def _create_regression_subtasks(
    ...
    """Create scoped subtasks on the regression fix task."""
    ...
    for subtask_id, description, order, subtask_type in subtasks:
        try:
            ...
            logger.warning(
                ...
                extra={"task_id": task_id, "subtask_id": subtask_id, "error": str(e)},
            )
...
def create_regression_fix_task(
    ...
    Args:
        ...
        failure_detail: Truncated validation failure output
