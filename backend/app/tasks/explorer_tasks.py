...
@shared_task(name="summitflow.discover_page_sub_elements")  # type: ignore[untyped-decorator]
def discover_page_sub_elements(project_id: str) -> dict[str, Any]:
    """Discover sub-elements (tabs, accordions, etc.) for all page entries.

    Runs browser-based discovery to find interactive elements that need
    individual verification during design audits.
    ...
    from ..services.evidence.sub_element_discoverer import discover_and_store

    logger.info("sub_element_discovery_started", project_id=project_id)
    ...
    try:
        ...
        pages = explorer_entries.get_pages_for_health_check(project_id)
        if not pages:
            return {"status": "success", "message": "No pages to scan", "scanned": 0}
        ...
        for i, page in enumerate(pages):
            ...
            try:
                ...
                count = asyncio.get_event_loop().run_until_complete(
                    discover_and_store(project_id, entry_id, url)
                )
                ...
                if count > 0:
                    logger.info(
                        "sub_elements_discovered",
                        path=path,
                        count=count,
                    )

            except Exception as e:
                logger.error("sub_element_discovery_failed", path=path, error=str(e))
                results.append(
                    {
                        "path": path,
                        "error": str(e),
        ...
        logger.info(
            "sub_element_discovery_complete",
            project_id=project_id,
        ...
        logger.error("sub_element_discovery_failed", project_id=project_id, error=str(e))
        return {"status": "error", "project_id": project_id, "error": str(e)}


def _run_ba_check(url: str, timeout: int = 30) -> dict[str, Any]:
