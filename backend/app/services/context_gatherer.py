...
def gather_all_context(project_id: str, raw_request: str) -> dict[str, Any]:
    ...
    total = sum(
        _estimate_tokens(context[key])
        for key in ["rules", "docs", "memory", "explorer", "capabilities"]
    )
    context["total_tokens"] = total

    # Log summary
    logger.info(
        "Gathered context for %s: rules=%d, docs=%d, memory=%d, explorer=%d, caps=%d (total ~%d tokens)",
        project_id,
