...
if TYPE_CHECKING:
    from . import OrchestratorService
...
async def do_coordinate(
    ...
    orch = orchestrator
    orch._set_state(ExecutionState.CLAIMING)
    await orch._send_log("info", f"Claiming task {task_id}")

    worker_id = worker_id or f"orchestrator-{orch.project_id}"
    ...
    if not claimed:
        ...
        await orch._send_log("error", result.error)
        return result

    logger.info("task_claimed", task_id=task_id, worker_id=worker_id)
    await orch._send_log("info", f"Task claimed by {worker_id}")
    ...
    if task:
        ...
        if complexity == "COMPLEX":
            ...
            if "architecture" in (labels or []) or requires_human_review(task):
                ...
                result.state = ExecutionState.FAILED
                await orch._send_log("info", "COMPLEX task routed to human review")
                return result

    orch._set_state(ExecutionState.SETTING_UP)
    await orch._send_log("info", "Creating isolated worktree")

    try:
        worktree_info = orch.worktree_manager.create_worktree(
            orch.project_id,
            task_id,
        )
        orch._current_worktree_path = worktree_info.path
        logger.info(
            "worktree_created",
            task_id=task_id,
        ...
        await orch._send_log("info", f"Worktree created: {worktree_info.branch}")
    ...
    if not subtasks:
        ...
        cleanup_on_failure(
            ...
            orch.worktree_manager,
            orch.project_id,
        )
        orch._current_worktree_path = None
        await orch._send_log("error", result.error)
        return result

    pending_subtasks = [s for s in subtasks if not s.get("passes")]
    await orch._send_log(
        "info", f"Found {len(pending_subtasks)}/{len(subtasks)} pending subtasks"
    )

    orch._set_state(ExecutionState.EXECUTING)
    await orch._send_model_change(AGENT_WORKER, "Starting with agent:coder worker")

    for subtask in pending_subtasks:
        if orch._interrupted:
    ...
    orch._set_state(ExecutionState.REVIEWING)
    await orch._send_log("info", "All subtasks complete, creating draft PR")
    ...
    await trigger_opus_review(task_id, pr_url, orch._send_log)

    result.success = True
    result.state = ExecutionState.COMPLETED
    await orch._send_log("info", "Orchestration complete - Opus review triggered")
...
async def execute_subtask(
    ...
    await orch._send_log("info", f"Starting subtask {subtask_id}: {str(description)[:50]}...")
    ...
    for attempt in range(orch.MAX_RETRIES):
        if orch._interrupted:
        ...
        if consecutive_failures >= orch.STUCK_THRESHOLD:
            model = AGENT_SUPERVISOR
            await orch._send_model_change(
                AGENT_SUPERVISOR,
        ...
        await orch._send_log(
            "info",
            f"Attempt {attempt + 1}/{orch.MAX_RETRIES} with {model}",
        )
        ...
        try:
            success, error = await dispatch_to_worker(
                subtask=subtask,
                model=model,
                effective_repo_path=orch.effective_repo_path,
                chat_messages=orch._chat_messages,
                send_log=orch._send_log,
            )

            if success:
                commit_message = f"auto({subtask_id}): {str(description)[:50]}"
                committed = orch.worktree_manager.commit_in_worktree(
                    orch.project_id,
                    task_id,
                    commit_message,
                )
                ...
                last_error = error
                consecutive_failures += 1
                await orch._send_log("warning", f"Attempt {attempt + 1} failed: {error}")

        except Exception as e:
            last_error = str(e)
            consecutive_failures += 1
            logger.error("subtask_execution_error", subtask_id=subtask_id, error=str(e))
            await orch._send_log("error", f"Execution error: {e}")

    result.error = f"Exhausted {orch.MAX_RETRIES} retries. Last error: {last_error}"
    return result
