...
class QuickPlanRequest(BaseModel):
    """Request for quick plan generation."""

    title: str
    description: str
    template: str  # bug-fix, add-endpoint, add-component
    params: dict[str, str] = {}


@router.post("/projects/{project_id}/tasks/quick-plan")
async def generate_quick_plan(
    ...
    Templates available:
    - bug-fix: Fix a bug with minimal changes
    - add-endpoint: Add a new API endpoint
    - add-component: Add a new frontend component

    Args:
        project_id: Project ID (used for context)
        body: Quick plan request with title, description, template, and params

    Returns:
        Complete plan.json structure ready for import
    """
    from ...services.quick_plan import generate_plan
