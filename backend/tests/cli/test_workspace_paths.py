"""Tests for workspace-aware lane path helpers."""
...
def test_get_lanes_base_dir_prefers_btrfs_workspace(monkeypatch, tmp_path: Path) -> None:
    from cli.lib.workspace_paths import get_lanes_base_dir

    workspace_root = tmp_path / "srv" / "workspaces"
    workspace_root.mkdir(parents=True)
    monkeypatch.setenv("ST_WORKSPACES_ROOT", str(workspace_root))

    result = get_lanes_base_dir("summitflow")
