"""Reaching a SummitFlow-managed project's own API from ``st``.

Some managed projects expose their endpoints at the application root rather than
under SummitFlow's ``/projects/<id>`` router, so :class:`cli._client_base.
BaseHTTPClient` — which scopes every URL that way — cannot address them. This
module carries the small amount of machinery those surfaces need: find the
project's backend wherever the operator happens to be standing, and speak plain
JSON to it.

Resolution precedence, highest first:

1. the surface's environment variable (e.g. ``ST_JOBS_API_URL``)
2. ``ports.json`` ``api_url`` / ``backend_port`` of the nearest checkout
3. ``project.identity.json`` ``runtime.backend_port`` of the same checkout
4. the SummitFlow project registry — works from any cwd
5. ``--remote`` → ``hosts.production_api`` from identity
6. the surface's declared default

Environment first because an operator overriding it means it; the local checkout
next because standing in a project is a statement about which one you mean; the
registry after that because it is correct but costs an HTTP call; and remote last
because a local backend, when one is running, is the one whose data you just
changed.

``cli/_portfolio_client.py`` predates this module and implements the same stack
inline. It is left alone deliberately — it works, and rewriting a shipped surface
is not a side effect a new one should have.
"""
...
class ProjectApi:
    """Where one managed project's API lives, and how to find it."""
...
class ProjectApiConnectError(Exception):
    """The project's backend could not be reached.

    Carries the URL that was tried so the command layer can name it in the
    unreachable envelope without re-deriving the resolution.
    """
...
def _walk_for_repo(api: ProjectApi, start: Path) -> Path | None:
    """Nearest ancestor of ``start`` that is this project's checkout."""
...
def _registry_root(api: ProjectApi) -> Path | None:
    """The project's checkout path according to the SummitFlow registry."""
    try:
    ...
    try:
        with httpx.Client(timeout=5.0) as client:
            response = client.get(f"{DEFAULT_API_BASE}/projects/{api.project_id}")
...
def resolve_api_url(
    ...
    """Resolve a managed project's API base URL. ``cwd`` is exposed for tests."""
    ...
    registry_root = _registry_root(api)
    ...
    if remote:
        remote_url = _from_remote(api, start)
        if remote_url:
            return ResolvedURL(url=remote_url, source="remote")

    return ResolvedURL(url=api.default_url, source="default")
