...
def _session_label(session_key: str) -> str:
    return session_key.removeprefix("session-")[:8]
...
def summarize_tool_input(tool_name: str, tool_input: dict[str, Any]) -> str:
    ...
    if tool_name == "glob":
        return f"pattern={tool_input.get('pattern', '')!r}"
...
def _format_event_header(event: dict[str, Any], verbose: bool) -> str:
    ...
    header = f"{color}[{event.get('turn', 0)}.{event.get('sequence', 0)}] {event_type}{_RESET}"
    if tool_name := event.get("tool_name"):
        header += f" ({tool_name})"
    if tokens := event.get("tokens"):
        header += f" [{tokens} tokens]"
    if verbose and (model := event.get("model_used")):
        header += f" [{model}]"
    return header
...
def _format_tool_use_content(event: dict[str, Any], verbose: bool) -> str:
    ...
    return summarize_tool_input(tool_name, tool_input) if tool_name else json.dumps(tool_input)[:120]
...
def build_memory_effectiveness_summary(
    ...
    for event in events:
        ...
        s = summary.setdefault(
            session_key,
            {"selected": 0, "indexed": 0, "selected_cited": 0, "total_cited": 0,
        ...
        if event_type == "memory_inject":
            s["selected"] = int(s["selected"]) + int(tool_input.get("reference_selected_count") or 0)
            s["indexed"] = int(s["indexed"]) + int(tool_input.get("reference_index_count") or 0)
            selected_refs_by_session[session_key] = _selected_refs_from_memory_inject(event)
        elif event_type == "memory_cite":
            cited = [str(u) for u in (tool_input.get("uuids") or []) if u]
            selected_uuids = selected_refs_by_session.get(session_key, set())
            s["total_cited"] = int(s["total_cited"]) + len(cited)
            s["selected_cited"] = int(s["selected_cited"]) + sum(1 for u in cited if u in selected_uuids)

    return {
        key: val for key, val in summary.items()
        if any(int(val[k]) > 0 for k in ("selected", "indexed", "selected_cited", "total_cited"))
    }


def _rate_str(cited: int, selected: int) -> str:
    return f"{round(cited / selected * 100):d}%" if selected > 0 else "0%"
...
def _render_memory_effectiveness_summary(
    ...
    lines = [" Memory:"]
    ordered_keys = [sid for sid in session_ids if sid in summary] if session_ids else list(summary.keys())

    if len(ordered_keys) == 1:
        item = summary[ordered_keys[0]]
        s, i, c = int(item["selected"]), int(item["indexed"]), int(item["selected_cited"])
        lines.append(f" refs selected={s} | index={i} | selected cited={c}/{s} ({_rate_str(c, s)})")
        return lines

    ts = sum(int(summary[k]["selected"]) for k in ordered_keys)
    ti = sum(int(summary[k]["indexed"]) for k in ordered_keys)
    tc = sum(int(summary[k]["selected_cited"]) for k in ordered_keys)
    lines.append(f" total selected={ts} | index={ti} | selected cited={tc}/{ts} ({_rate_str(tc, ts)})")
    for key in ordered_keys:
        item = summary[key]
        s, i, c = int(item["selected"]), int(item["indexed"]), int(item["selected_cited"])
        lines.append(f" {item['session_label']!s}: refs={s} | index={i} | selected cited={c}/{s} ({_rate_str(c, s)})")
    return lines
    ...
    for event in events:
        ...
        if event.get("event_type") in {"memory_inject", "memory_cite"}:
            ...
            event_for_display["content"] = _format_memory_event_content(
                event, verbose, selected_refs_by_session.get(session_key),
            )
