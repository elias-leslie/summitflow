...
Reads, merges, and deletes task-family branches left over from the legacy
branch-checkpoint era. New work commits direct to main; legacy checkpoint
branches can still be completed safely.
"""
...
def _merge_failure_output(error: subprocess.CalledProcessError) -> str:
    parts = [str(part).strip() for part in (error.stderr, error.stdout) if str(part or "").strip()]
    return "\n".join(parts)
...
def _format_merge_failure(branch: str, error: subprocess.CalledProcessError, cwd: str | None, recovery: str, conflicts: list[str] | None = None) -> str:
    output = _merge_failure_output(error)
...
def _record_task_merge_conflict(task_id: str, task_branch: str, base_branch: str, conflicts: list[str], error_output: str) -> None:
    if not conflicts:
        return
    try:
        ...
        update_task_status(task_id, "failed", error_message=f"Merge conflict in {len(conflicts)} file(s)", validate_transition=False)
        log_task_event(task_id, f"Merge conflict detected in {len(conflicts)} file(s): {', '.join(conflicts[:5])}")
...
def merge_subtask_branch(task_id: str, subtask_id: str, project_id: str | None = None) -> bool:
    """No-op. Subtask work is committed direct to main via st commit."""
...
def merge_task_branch(task_id: str, project_id: str | None = None) -> bool:
    ...
    try:
        ...
        print(_format_merge_failure(branch_name, exc, repo_cwd, recovery=f"st git resolve-conflict {task_id}", conflicts=conflicts), file=sys.stderr)
        sys.exit(1)
