...
class _TaskCrudMixin(_MixinBase):
    """Core task CRUD and listing operations."""
    ...
    def get_task_logs(self, project_id: str, task_id: str) -> dict[str, Any]:
        return tasks_ops.get_task_logs(
            self._client, self._global_url, self._handle_response, project_id, task_id,
        )
...
class _TaskWorkflowMixin(_MixinBase):
    ...
    def update_status(
        ...
        return tasks_ops.update_status(
            self._client, self._url, self._handle_response,
            task_id, status, error_message, reason, skip_gates,
        )

    def close_task(
        self, task_id: str, reason: str | None = None, skip_gates: bool = False,
    ) -> dict[str, Any]:
...
class DependencyOperationsMixin(_MixinBase):
