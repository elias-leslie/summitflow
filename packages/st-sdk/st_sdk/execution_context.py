"""Repository and checkout context helpers shared with ST extensions."""
...
def resolve_checkout_root(cwd: Path | None = None) -> Path | None:
    ...
    return _resolve_git_path(
        ["git", "rev-parse", "--path-format=absolute", "--show-toplevel"], cwd
    )
...
def resolve_checkout_project_id(cwd: Path | None = None) -> str | None:
    """Resolve the checkout's project id without considering CLI overrides."""
