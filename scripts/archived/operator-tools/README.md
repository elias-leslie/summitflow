# Archived Operator Tool References

These files are reference copies only. They are not public operator commands and
must not be added to `PATH`, linked into `~/bin`, or used from prompts/memory.
...
- `st vm` replaces Proxmox VM wrappers.
- `st setup` replaces setup wrappers.
