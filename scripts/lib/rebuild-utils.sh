...
build_frontend() { local d="$PROJECT_DIR/frontend"; [ ! -d "$d" ] && d="$PROJECT_DIR"; cd "$d" || return 1; log "Building..."; npm run build --silent 2>&1 | tail -5 && log_success "Built" || { log_error "Build failed"; return 1; }; }
...
verify_frontend() { local p="${1:-$FRONTEND_PORT}"; log "Checking frontend ($p)..."; for i in $(seq 1 15); do local s=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:$p/" 2>/dev/null); [ "$s" = "200" ] && { log_success "Frontend OK ($s)"; return 0; }; sleep 1; done; log_error "Frontend failed"; return 1; }
