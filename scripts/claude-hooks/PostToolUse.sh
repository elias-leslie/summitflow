...
#
# Environment variables from Claude Code (passed via stdin as JSON):
...
# Get session ID (use Claude Code session ID or generate from timestamp)
...
if [ -n "${SUMMITFLOW_PROJECT_ID:-}" ]; then
    ...
    PROJECT_ID=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" | tr '[:upper:]' '[:lower:]')
else
