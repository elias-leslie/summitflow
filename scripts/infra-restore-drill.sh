...
# Find the extracted content — may be under infrastructure/ prefix
EXTRACT_ROOT="$DRILL_DIR"
if [ -d "$DRILL_DIR/infrastructure" ]; then
    EXTRACT_ROOT="$DRILL_DIR/infrastructure"
fi

# ── 1. PostgreSQL: load pg_dumpall into disposable container ──
...
# ── 2. Redis: load RDB into disposable container ──
...
if [ -n "$REDIS_RDB" ]; then
    ...
    if [ "$HEADER" = "REDIS" ]; then
        # Start disposable redis with the RDB
...
# ── 3. Hatchet config: verify files exist and YAML is parseable ──
...
if [ -n "$HATCHET_DIR" ] && [ -d "$HATCHET_DIR" ]; then
    ...
    if [ "${hatchet_files:-0}" -gt 0 ]; then
        # Check for server.yaml (primary config)
...
# ── 4. Config files: verify env files contain expected content ──
