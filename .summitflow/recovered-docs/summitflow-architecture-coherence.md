...
| Cross-project dev utilities | App-specific utilities |
...
**Example mistake:** Building a "Tasks Explorer" (hierarchical Celery task viewer) in a project instead of SummitFlow. This is dev tooling for understanding task architecture, not operational monitoring. Belongs in SummitFlow as cross-project feature.
...
3. **CREATE** task if not exists:
   ```bash
   st create "Arch: <clear description>" -t task -p 2 \
...
Architecture violations are automatically captured as observations for pattern learning. When violations are fixed, the approach becomes a learned pattern that helps prevent future occurrences.
